...
const { search } = require("../routes/admin.js");
module.exports = {
  ...
  IssueBook: async (req, res) => {
    try {
      ...
      res.json({ success: true, message: "Successfully Book Issue updated" });
    } catch (err) {
      console.log(err);
  ...
  UpdateComment: async (req, res) => {
    ...
    try {
      await CommentModel.findByIdAndUpdate(comment_id, comment_text);
    ...
    try {
      ...
      if (id) {
        const book = await bookModel
          .find(id)
          .populate({ path: "Book", select: { _id: 0, title: 1 } });
